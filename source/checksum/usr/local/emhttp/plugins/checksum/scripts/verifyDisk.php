...
$diskPassed = str_replace("disk","",$sharePassed);

$percent = $allSchedule['Disk'][$sharePassed]['PercentToCheck'];
if ( ! $percent )
{
  $percent = 10;
}

$lastPercent = $allSchedule['Disk'][$sharePassed]['LastChecked'];
if ( ! $lastPercent )
{
  $lastPercent = 0;
}
...
$commandLine = '/usr/local/emhttp/plugins/checksum/scripts/verify.php "'.$diskPassed.'" '.$percent.' '.$lastPercent;
...
$allSchedule['Disk'][$sharePassed]['LastStatus'] = $returnFlag;
$allSchedule['Disk'][$sharePassed]['LastChecked'] = $newLastPercent;
$allSchedule['Disk'][$sharePassed]['LastCheckedDate'] = date("M d Y");
